...
field = gmt_embedding_2(oldfield2, tas_rescaled, gfp; N) # gmt_embedding(oldfield, tas_rescaled, gfp)
...
for epoch in ProgressBar(1:epochs)
    ...
    if epoch % 100 == 0
        @info "saving model"
        CliMAgen.save_model_and_optimizer(Flux.cpu(score_model), Flux.cpu(score_model_smooth), opt, opt_smooth, "temp_$epoch.bson")
    end
end

hfile = h5open("temperature_losses.hdf5", "w")
