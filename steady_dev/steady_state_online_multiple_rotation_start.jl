...
checkpoint_path = "checkpoint_rotations_steady_online_timestep_160000.bson"# "checkpoint_large_temperature_vorticity_humidity_divergence_pressure_timestep.bson" # "checkpoint_large_temperature_vorticity_humidity_divergence_timestep_Base.RefValue{Int64}(10000).bson" # "checkpoint_large_temperature_vorticity_humidity_divergence_timestep.bson" # "checkpoint_large_temperature_vorticity_humidity_divergence_timestep_Base.RefValue{Int64}(130000).bson"
    ...
    while j[] <= nsteps
        if all_closed(gates)
            ...
            if j[]%40000 == 0 
                tmp = j[]
                @info "saving model"
                CliMAgen.save_model_and_optimizer(Flux.cpu(score_model), Flux.cpu(score_model_smooth), opt, opt_smooth, "checkpoint_rotation_start_steady_online_timestep_$tmp.bson")
            end
        else
            sleep(SLEEP_DURATION)
        end
...
hfile = h5open("losses_online_rotation_start.hdf5", "w")
hfile["losses"] = losses
