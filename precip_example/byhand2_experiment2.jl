...
# load data
FT = Float32
hfile = h5open("/nobackup1/users/sandre/GaussianEarth/pr_field_1.hdf5", "r")
...
field = gmt_embedding_8(oldfield2, tas_rescaled, gfp; N) # gmt_embedding(oldfield, tas_rescaled, gfp)
...
for epoch in ProgressBar(1:epochs)
    ...
    if epoch % 100 == 0
        @info "saving model"
        CliMAgen.save_model_and_optimizer(Flux.cpu(score_model), Flux.cpu(score_model_smooth), opt, opt_smooth, "experiment2_pr_$epoch.bson")
    end
end

hfile = h5open("pr_losses_2.hdf5", "w")
