...
model_toml = "Model6.toml"
experiment_toml = "Experiment6.toml"
FT = Float32
...
old_ctrain = train[:, :, (noised_channels+1):(noised_channels+context_channels), shuffle(1:nsamples)]
...
τ0 = reshape(gfp(1.0), (size(xtrain)[1], size(xtrain)[2], 1, 1))
if length(inds) == 1
