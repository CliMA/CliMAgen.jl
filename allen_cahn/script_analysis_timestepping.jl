...
τ = round(Int, t * 60)
acov_true = mean(rtseries[:, :, 1:nsamples, 1:end-τ] .* rtseries[:, :, 1:nsamples, 1+τ:end]) - mean(rtseries[:, :, 1:nsamples, 1:end-τ]) * mean(rtseries[:, :, 1:nsamples, 1+τ:end])
acov = mean(samples[:, :, 1, :] .* samples[:, :, 2, :]) - mean(samples[:, :, 1, :]) * mean(samples[:, :, 2, :])
println("generative autocovariance is ", acov)
println("true autocovariance is ", acov_true)