...
for i in 1:2
    ...
    heatmap!(ax,lon, lat, ctrain[:,:,1,i]; colorrange, colormap = :thermometer, interpolate = true)
    ax = Axis(fig[i, 2]; title = "ai next timestep sample $i")
    heatmap!(ax,lon, lat, samples[:,:,1,i]; colorrange, colormap = :thermometer, interpolate = true)
