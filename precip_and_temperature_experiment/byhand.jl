...
inds = 1:251
N = length(inds)
field = ensemble_average_context(oldfield)
...
for epoch in ProgressBar(1:epochs)
    ...
    if epoch % 100 == 0
        @info "saving model"
        CliMAgen.save_model_and_optimizer(Flux.cpu(score_model), Flux.cpu(score_model_smooth), opt, opt_smooth, "temp_pr_$epoch.bson")
    end
end

hfile = h5open("temp_pr_losses.hdf5", "w")
