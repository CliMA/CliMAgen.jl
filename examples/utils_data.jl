...
"""
    get_data_response2dturbulence(batchsize, resolution, nonlinearity;
                       f = 1.0,
                       time_res = 1,
                       FT=Float32,
...
Obtains the 2D turbuelence low resolution dataset, 
...
"""
function get_data_response2dturbulence(batchsize;
    nonlinearity = :strong,
    f=0.8,
    FT=Float32,
    rng=Random.GLOBAL_RNG,
    ...
    xtrain = Turbulence2DLowRes(:train; f=f, Tx=FT, resolution=resolution, nonlinearity=nonlinearity)[:]
    xtest  = Turbulence2DLowRes(:test; f=f, Tx=FT, resolution=resolution, nonlinearity=nonlinearity)[:]
