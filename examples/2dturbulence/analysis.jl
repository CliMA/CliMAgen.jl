...
function run_analysis(params; FT=Float32, logger=nothing)
    ...
    img_plot(samples[:, :, [1], 1:nimages], savedir, "$(sampler)_images_ch1.png", logger=logger)
    img_plot(xtrain[:, :, [1], 1:nimages], savedir, "train_images_ch1.png", logger=logger)
    img_plot(samples[:, :, [2], 1:nimages], savedir, "$(sampler)_images_ch2.png", logger=logger)
    img_plot(xtrain[:, :, [2], 1:nimages], savedir, "train_images_ch2.png", logger=logger)
