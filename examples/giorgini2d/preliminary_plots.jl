...
surface!(ax, reverse(x), reverse(y), dx0', colormap = :viridis, rotation = pi/2)
save("perturbation_$(sigma[1]).png", fig)
...
for (i, gl) in enumerate(glist)
    ...
    lr_smooth = create_response_smooth(dx0,linear_indices,linear_response[:,1,:])[1:8:end,:] # effect of perturbation of pixel 1 on pixel i
    srh_smooth = create_response_smooth(dx0,score_indices,score_response_hack[:,1,:])[1:8:end,:] # effect of perturbation of pixel 1 on pixel i
    nr_smooth = create_response_smooth(dx0,numerical_indices,numerical_response[:,:])[1:8:end,1:4:end] # effect of perturbation of pixel 1 on pixel i
...
save("responses_smooth_$(sigma[1]).png", fig)