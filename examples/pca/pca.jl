...
@with_kw struct HyperParams
    ...
    img_size = 256
    input_channels = 1
    lr_gen = Float32(0.00001) #Float32(0.0002)
    lr_dscr = Float32(0.00001) #Float32(0.0002)
    λ = Float32(10.0)
    λid = Float32(0.0)
    color_format = Gray
end
...
function train(; kwargs...)
    ...
    pca_lores = fit(PCA, data_lores, maxoutdim=hparams.num_latent_dim, pratio=1.0)
    ...
    data_lores = permutedims(data_lores[:, :, :], (1, 3, 2)) |> device
    ...
    pca_hires = fit(PCA, data_hires, maxoutdim=hparams.num_latent_dim, pratio=1.0)
    ...
    data_hires = permutedims(data_hires[:, :, :], (1, 3, 2)) |> device
    ...
    gen_hires = UNetGenerator1D(hparams.input_channels) |> device # Generator For lores->hires
    gen_lores = UNetGenerator1D(hparams.input_channels) |> device # Generator For hires->lores
    dscr_hires = PatchDiscriminator1D(hparams.input_channels) |> device # Discriminator For lores domain
    dscr_lores = PatchDiscriminator1D(hparams.input_channels) |> device # Discriminator For hires domain
    ...
    for epoch in 1:hparams.num_epochs
        ...
        for (lores, hires) in data
            ...
            if num_examples % hparams.checkpoint_freq == 0
                ...
                file_last = hparams.output_path * hparams.exp_name * "/checkpoint_latest.bson"
                networks_cpu = (gen_lores, gen_hires, dscr_lores, dscr_hires) |> cpu
                @save file_last networks_cpu

                # Generate fake latent images
                lores_sample, hires_sample = lores[:, :, [1]], hires[:, :, [1]] # Take first image from each batch
                hires_fake = gen_hires(lores_sample)
                lores_fake = gen_lores(hires_sample)
                ...
                lores_fake = lores_fake |> cpu
                hires_fake = hires_fake |> cpu
                ...
                prefix_path = hparams.output_path * hparams.exp_name * "/training/" * "image_$num_examples"
