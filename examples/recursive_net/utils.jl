...
function get_dataloader(path; field="moisture", split_ratio=0.5, batch_size=1, nsamples=1000, dev=cpu)
    ...
    # Build pairs of images for autoregressive training
    X_lo_res = cat(X_lo_res[:, :, :, 1:end-16], X_lo_res[:, :, :, 17:end], dims=3)
    X_hi_res = cat(X_hi_res[:, :, :, 1:end-16], X_hi_res[:, :, :, 17:end], dims=3)
