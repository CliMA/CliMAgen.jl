...
hfile = h5open("/home/sandre/Repositories/CliMAgen.jl/examples/correlated_ou/OUdata.hdf5")
data = HDF5.read(hfile, "x")
# close(hfile)

# reshape to 32x32x1xnobs_1
nobs_1 = batchsize_1 * nbatches #size(data)[end]
data = reshape(data[:,1:nobs_1], (32,32,1,nobs_1))

# carry out preprocessing
# data .= apply_preprocessing(data, scaling)
# Allocate memory for the score
data_score = similar(data)
data_score .= FT(0)
# set up model
...
for batch in ProgressBar(1:nbatches)
    ...
    gpu_data = device(data[:,:,[1],batch_indices])
    data_score[:,:,[1],batch_indices] .= Array(CliMAgen.score(model, gpu_data, t0))
end

@info "saving"
# write new hdf5 with preprocessed data and the score
hfile = h5open("/home/sandre/Repositories/CliMAgen.jl/response_function_ou_less_training.h5", "cw")
hfile["timeseries"] = data 
hfile["score"] = data_score 
close(hfile)
@info "done"

# scp sandre@tartarus.mit.edu:/home/sandre/Repositories/CliMAgen.jl/response_function_ou_more_training.h5 
# scp sandre@tartarus.mit.edu:/home/sandre/Repositories/CliMAgen.jl/examples/correlated_ou/output/checkpoint.bson .