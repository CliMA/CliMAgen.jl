...
function generate_score(alpha,beta,gamma,sigma;res=1)
    ...
    if isfile("output/checkpoint_$(alpha)_$(beta)_$(gamma)_$(sigma).bson") rm("output/checkpoint_$(alpha)_$(beta)_$(gamma)_$(sigma).bson") end

    if isfile("output/losses_$(alpha)_$(beta)_$(gamma)_$(sigma).bson") rm("output/losses_$(alpha)_$(beta)_$(gamma)_$(sigma).bson") end

    checkpoint_path = joinpath(savedir, "checkpoint.bson")
    loss_file = joinpath(savedir, "losses_$(alpha)_$(beta)_$(gamma)_$(sigma).txt")
    ...
    mv("output/checkpoint.bson", "output/checkpoint_$(alpha)_$(beta)_$(gamma)_$(sigma).bson")
    ...
    # data = read(hfile["timeseries"])[:,:,1:100000]
    # close(hfile) 
    # data = data[:,:,1:end]
    # times = [1:size(data)[3]...]  
    # t0 = 0.  
