...
"""
function setup_sampler(device, num_images=NSAMPLE, num_steps=500, ϵ=1.0f-3, exp_fac=EXPFAC)
    t = ones(Float32, num_images) |> device
    init_x = (
        randn(Float32, (28*exp_fac, 28*exp_fac, 1, num_images)) .*
...
https://yang-song.github.io/blog/2021/score/#how-to-solve-the-reverse-sde
"""
function predictor_corrector_sampler(model, init_x, time_steps, Δt, snr=0.16f0, k=BIGK, ex=METRIC, masker=MASKER)
    x = mean_x = init_x
    ...
    @showprogress "Predictor Corrector Sampling" for time_step in time_steps
        ...
        grad = model(x, batch_time_step) .+ guide
        ...
        mean_x = x .+ expand_dims((g .^ 2), 3) .* grad .* Δt
...
⟹ `g(u, p, t` = σᵗ
"""
function DifferentialEquations_problem(model, init_x, time_steps, Δt, k=BIGK, ex=METRIC, masker=MASKER)
    # extreme guidance step
    nx = size(init_x)[1]
    mask = zero(init_x) .+ 1
    mask = mask ./ (nx * nx)
    ...
    function f(u, p, t)
        ...
        # score from training with diffusion
        if ex == :mean
            guide = k .* mask
        elseif ex == :var
            guide = 2k .* (u  .- mean(u)) .* mask
        end
        score = model(u, batch_time_step) .+ guide
        return (
