...
FT = Float32
exp_name = "moist2d"
input_path = "../../../Downscaling_old.jl/data/"
output_path = "./output/"
mkpath(output_path)
eval_freq = 350
checkpoint_freq = eval_freq
device = gpu
num_examples = 700 #700
num_epochs = 100
batch_size = 1
img_size = 256
input_channels = 1
...
# set up logger
EXAMPLE_NAME = "turbulence_2d_old/cyclegan"
...
# Define models
generator_A = UNetGenerator2D(input_channels) |> device # Generator For A->B
generator_B = UNetGenerator2D(input_channels) |> device # Generator For B->A
discriminator_A = PatchDiscriminator2D(input_channels) |> device # Discriminator For Domain A
discriminator_B = PatchDiscriminator2D(input_channels) |> device # Discriminator For Domain B
...
function training()
    # Load data
    dataA = load_dataset(input_path * exp_name * "/trainA/", img_size, FT, color_format)[:, :, :, 1:num_examples]
    dataB = load_dataset(input_path * exp_name * "/trainB/", img_size, FT, color_format)[:, :, :, 1:num_examples]
    ...
    for epoch in 1:num_epochs
        ...
        for (batch_idx, (a, b)) in enumerate(data)
            a, b = (normalize(a), normalize(b)) |> device
            train_step!(opt_gen, opt_dis, a, b)
        ...
        if LOGGING
            ...
            a, b = first(data)
            a, b = (FT.(a), FT.(b)) |> device
            b_fake = generator_A(a)
            a_fake = generator_B(b)
            b_rec = generator_A(a_fake)
            a_rec = generator_B(b_fake)
