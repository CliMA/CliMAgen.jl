include("DiffusionModels.jl")

using Flux
using Test

@testset begin "Models"
    m = DiffusionModels.VarianceExplodingSDE()

    # test constructor
    @test m.σ_min == 0.01f0
    @test m.σ_max == 50.0f0

    # test t_end
    @test DiffusionModels.t_end(m) == 1

    # test drift
    t = rand(3, 10)
    @test DiffusionModels.drift(m, t) == zero(t)

    # test sigma & diffusion
    t = rand(3, 10)
    @test DiffusionModels.sigma(m, t) == @. m.σ_min * (m.σ_max / m.σ_min)^t
    @test DiffusionModels.diffusion(m, t) == @. m.σ_min * (m.σ_max / m.σ_min)^t * sqrt(2 * (log(m.σ_max) - log(m.σ_min)))

    # test dummy score
    x, t = rand(3), rand(3)
    @test DiffusionModels.score(m, x, t) == x

    # test forward_sde
    f, g = DiffusionModels.forward_sde(m)
    x, t = rand(11), rand()
    @test all(f(x, t) .== DiffusionModels.drift(m, t))
    @test all(g(x, t) .== DiffusionModels.diffusion(m, t))

    # test reverse_sde
    f, g = DiffusionModels.reverse_sde(m)
    x, t = rand(3), rand()
    @test f(x, t) == DiffusionModels.drift(m, t) .- DiffusionModels.diffusion(m, t)^2 .* DiffusionModels.score(m, x, t)
    @test all(g(x, t) .== DiffusionModels.diffusion(m, t))

    # test reverse_ode
    f = DiffusionModels.reverse_ode(m)
    x, t = rand(3), rand()
    @test f(x, t) == DiffusionModels.drift(m, t) .- DiffusionModels.diffusion(m, t)^2 .* DiffusionModels.score(m, x, t) ./ 2
end

@testset begin "Losses"
    m = DiffusionModels.VarianceExplodingSDE()
    x = rand(3, 7, 8, 3, 25)
    @test DiffusionModels.score_matching_loss(m, x) isa Real
end

@testset begin "Networks"
    net = DiffusionModels.NCNN()
    ps = Flux.params(net)
    for k in 1:2:5
        x = rand(Float32, k*28, k*28, 1, 11)
        t = rand(Float32)

        # forward pass
        @test net(x, t) |> size == size(x)

        # backward pass of dummy loss
        loss, grad = Flux.withgradient(ps) do
            sum(net(x, t).^2)
        end
        @test loss isa Real
    end
end