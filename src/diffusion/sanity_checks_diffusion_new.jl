...
"""
function predictor_corrector_sampler(model::DiffusionModels.AbstractDiffusionModel,  init_x, time_steps, Δt, snr=0.16f0)
    x = mean_x = init_x
    ...
    @showprogress "Predictor Corrector Sampling" for time_step in time_steps
        ...
        grad = DiffusionModels.score(model, x, batch_time_step)
        ...
        mean_x = x .+ DiffusionModels.expand_dims((g .^ 2), 3) .* grad .* Δt
        x = mean_x + sqrt.(DiffusionModels.expand_dims((g .^ 2), 3) .* Δt) .* randn(Float32, size(x))
    end
    return mean_x
end
...
function setup_sampler(model::DiffusionModels.AbstractDiffusionModel, device; num_images=5, num_steps=500, ϵ=1.0f-3)
    ...
    _, σ_T = DiffusionModels.marginal_prob(model, zero(init_z), t)
