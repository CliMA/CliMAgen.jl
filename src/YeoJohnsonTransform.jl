...
"""
function transform_array(x; optim_args...)
...
"""
    transform(x::FT, λ) where {FT}

Transform an array using Yeo-Johnson method with the provided power parameter λ. 
"""
function transform(x::FT, λ) where {FT <: AbstractFloat} 
    if x >= 0
        x′ = λ ≈ 0 ? log(x + 1) : ((x + 1)^λ - 1)/λ 
    else
        x′ = λ ≈ 2 ? -log(-x + 1) : -((-x + 1)^(2 - λ) - 1) / (2 - λ)
    end
    x′
end

"""
    inversetransform(x::FT, λ) where {FT <: AbstractFloat} 

Inverse transforms an array using Yeo-Johnson method with the provided power parameter λ. 
"""
function inversetransform(x::FT, λ) where {FT <: AbstractFloat}  
        if x >= 0
            x′ = λ ≈ 0 ? exp(x) - 1 : (λ*x+1)^(1/λ)-1 
        else
            x′ = λ ≈ 2 ? 1 - exp(-x) : 1 - (-(2-λ)*x + 1)^(1/(2-λ))
        end
    x′
end
