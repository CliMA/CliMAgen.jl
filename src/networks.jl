...
function DenoisingDiffusionNetwork(; kernel_size=3, nspatial=2, inchannels=1, channels=[64, 128, 128, 256], nembed=256, scale=30.0f0, p=0.1f0, σ=Flux.swish)
    ...
    return DenoisingDiffusionNetwork(
        ...
        downsample1=Downsampling(channels[1], nspatial),
            ...
            downsample2=Downsampling(channels[2], nspatial),
                ...
                downsample3=Downsampling(channels[3], nspatial),
                    ...
                    upsample4=Upsampling(channels[4], nspatial),
                ...
                upsample3=Upsampling(channels[3], nspatial),
            ...
            upsample2=Upsampling(channels[2], nspatial),
...
function Downsampling(nchannels::Int, nspatial::Int, factor::Int=2, kernel_size::Int=3)
    conv_kernel = Tuple(kernel_size for _ in 1:nspatial)
    return Conv(conv_kernel, nchannels => nchannels, stride=factor, pad=SamePad())
end
...
"""
function Upsampling(nchannels::Int, nspatial::Int, factor::Int=2, kernel_size::Int=3)
    conv_kernel = Tuple(kernel_size for _ in 1:nspatial)
    return Chain(
        Flux.Upsample(factor, :nearest),
        Conv(conv_kernel, nchannels => nchannels, pad=SamePad())
    )
end
